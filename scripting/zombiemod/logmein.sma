	...
	if (menu != MENU_NONE)
	{
		if (menu == MENU_MAIN)
		{
			...
			else
			{
				...
				g_autoLogin[id] = true;
				client_print(id, print_chat, "[LogMeIn] Auto login has been set.");
			}
...
public OnClientUserInfoChanged(id)
{
	...
	if (oldName[0])
	{
		...
		{
			client_print(id, print_chat, "* Change name is not allowed.");
...
public ShowLogmeinMenu(id)
{
	...
	switch (g_status[id])
	{
		...
		{
			len += formatex(menu[len], 511-len, " \y[\rUnregistered\y]^n^n");
		...
		{
			len += formatex(menu[len], 511-len, " \y[\rNot Logged In\y]^n^n");
			len += formatex(menu[len], 511-len, "\y1. \wLogin^n");
		...
		case USER_LOGGED_IN:
		{
			...
			len += formatex(menu[len], 511-len, "\y2. \wAuto login: %s^n", g_autoLogin[id] ? "\yOn" : "\dOff");
	...
	switch (key)
	{
		...
		case 1:
		{
			if (g_status[id] == USER_LOGGED_IN)
			{
				...
				client_print(id, print_chat, "[LogMeIn] Please enter your password. (enter empty value to disable)");
				ShowLogmeinMenu(id);
			}
		}
		case 8:
...
public ShowRegisterMenu(id)
{
	...
	len = formatex(menu, charsmax(menu), "\yRegister Menu \y[\w%n\y]^n^n", id);
	...
	len += formatex(menu[len], 511-len, "\y1. \wPassword: \y%s^n", !censored[0] ? "\dPress to enter" : censored);
	...
	len += formatex(menu[len], 511-len, "\y2. \wConfirm Password: \y%s^n", !censored[0] ? "\dPress to enter" : censored);
...
public ShowLoginMenu(id)
{
	...
	len = formatex(menu, charsmax(menu), "\yLogin Menu \y[\w%n\y]^n^n", id);
...
public TaskLogin(id)
{
	...
	ShowSyncHudMsg(id, g_hudSyncObj, "[%n]^nYou have %d seconds to login.", id, timeLeft);
...
stock bool:loginSubmit(id)
{
	...
	{
		g_wrong[id]++;
		client_print(id, print_chat, "[LogMeIn] Incorrect password. [%d/5]", g_wrong[id]);
		return false;
	}
